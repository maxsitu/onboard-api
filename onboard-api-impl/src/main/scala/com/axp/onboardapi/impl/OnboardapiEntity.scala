...
/**
 * This is an event sourced entity. It has a state, [[OnboardapiState]], which
 ...
 */
class OnboardapiEntity extends PersistentEntity {

  override type Command = OnboardapiCommand[_]
  override type Event = OnboardapiEvent
  override type State = OnboardapiState

  /**
   * The initial state. This is used if there is no snapshotted state to be found.
   */
  override def initialState: OnboardapiState = OnboardapiState("Hello", LocalDateTime.now.toString)
  ...
  override def behavior: Behavior = {
    case OnboardapiState(message, _) =>
      Actions()
        ...
        .onEvent {
          ...
          case (GreetingMessageChanged(newMessage), state) =>
            ...
            // the event.
            OnboardapiState(newMessage, LocalDateTime.now().toString)
...
object OnboardapiState {
   ...
   */
  implicit val format: Format[OnboardapiState] = Json.format
}

/**
 * This interface defines all the events that the OnboardapiEntity supports.
 */
sealed trait OnboardapiEvent extends AggregateEvent[OnboardapiEvent] {
  def aggregateTag: AggregateEventTag[OnboardapiEvent] = OnboardapiEvent.Tag
}

object OnboardapiEvent {
  val Tag: AggregateEventTag[OnboardapiEvent] = AggregateEventTag[OnboardapiEvent]
}

/**
 * An event that represents a change in greeting message.
 */
case class GreetingMessageChanged(message: String) extends OnboardapiEvent
...
/**
 * This interface defines all the commands that the OnboardapiEntity supports.
 */
sealed trait OnboardapiCommand[R] extends ReplyType[R]
 ...
 */
case class UseGreetingMessage(message: String) extends OnboardapiCommand[Done]
 ...
 * person.
 */
case class Hello(name: String) extends OnboardapiCommand[String]
 ...
 * application loader.
 */
object OnboardapiSerializerRegistry extends JsonSerializerRegistry {
  override def serializers: Seq[JsonSerializer[_]] = Seq(
    JsonSerializer[UseGreetingMessage],
    JsonSerializer[Hello],
    JsonSerializer[GreetingMessageChanged],
    JsonSerializer[OnboardapiState]
  )
}
